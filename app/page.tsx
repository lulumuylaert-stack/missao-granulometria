...
export default function Home() {
  ...
  const [cuFormula,setCuFormula]=useState(""), [cuInput,setCuInput]=useState(""), [cuMeaning,setCuMeaning]=useState("");
  const [ccFormula,setCcFormula]=useState(""), [ccInput,setCcInput]=useState(""), [ccMeaning,setCcMeaning]=useState("");
  ...
  const checkCoefficients=()=>{const number=(v:string)=>Number(v.replace(",","."));const ok=cuFormula==="D60 / D10"&&Math.abs(number(cuInput)-3.75)<.01&&cuMeaning==="solo uniforme"&&ccFormula==="(D30)² / (D10 · D60)"&&Math.abs(number(ccInput)-1.28)<.01&&ccMeaning==="1 < Cc < 3";if(ok){award(350);setAnswered(1);setFeedback("✅ Coeficientes calculados e interpretados. A areia não atende simultaneamente ao critério de bem graduada, pois Cu = 3,75 é menor que 4.")}else wrong("Revise fórmula, resultado e interpretação. Use os valores D10 = 0,08; D30 = 0,175 e D60 = 0,3 mm.")};
  const reset=()=>{setPhase(0);setScore(0);setLives(3);setFeedback("");setQ(Math.floor(Math.random()*5));setAnswered(0);setCurve(false);setTargets([]);setPlotStep(0);setCurveClass("");setCuFormula("");setCuInput("");setCuMeaning("");setCcFormula("");setCcInput("");setCcMeaning("");setTableAnswers(sample.rows.map(()=>["","",""]));setOrder([...sieves].sort(()=>Math.random()-.5))};
  return <main>
    ...
    <section className="stage">
      ...
      <article className="card">
        ...
        {phase===6 && <><h2>Localize e calcule os coeficientes</h2><p>Escolha um percentual. A linha sai do eixo vertical, encontra a curva e desce até o diâmetro no eixo horizontal.</p><Plot active guides={targets}/><div className="hunt">{[{k:"D10",v:.08},{k:"D30",v:.175},{k:"D60",v:.3}].map(o=><button className={targets.includes(o.k)?"found":""} key={o.k} onClick={()=>{if(!targets.includes(o.k)){setTargets([...targets,o.k]);award(150)}}}>{o.k}<small>{targets.includes(o.k)?`${fmt(o.v)} mm`:`mostrar linha de ${o.k.slice(1)}%`}</small></button>)}</div>{targets.length===3&&answered===0&&<div className="coefficient-lab"><div className="coefficient-card"><span>MISSÃO Cu</span><h3>Coeficiente de Não Uniformidade</h3><p>1. Selecione a fórmula:</p><div className="formula-options">{["D60 / D10","D10 / D60","D30 / D10"].map(x=><button className={cuFormula===x?"selected":""} onClick={()=>setCuFormula(x)} key={x}>{x}</button>)}</div><label>2. Calcule Cu <input aria-label="Resultado de Cu" value={cuInput} onChange={e=>setCuInput(e.target.value)} inputMode="decimal" placeholder="?"/></label><p>3. Interprete o resultado:</p><div className="formula-options">{["solo uniforme","solo medianamente uniforme","solo não uniforme"].map(x=><button className={cuMeaning===x?"selected":""} onClick={()=>setCuMeaning(x)} key={x}>{x}</button>)}</div></div><div className="coefficient-card"><span>MISSÃO Cc</span><h3>Coeficiente de Curvatura</h3><p>1. Selecione a fórmula:</p><div className="formula-options">{["(D30)² / (D10 · D60)","D60 / D10","D30 / (D10 · D60)"].map(x=><button className={ccFormula===x?"selected":""} onClick={()=>setCcFormula(x)} key={x}>{x}</button>)}</div><label>2. Calcule Cc <input aria-label="Resultado de Cc" value={ccInput} onChange={e=>setCcInput(e.target.value)} inputMode="decimal" placeholder="?"/></label><p>3. Em qual intervalo está Cc?</p><div className="formula-options">{["Cc < 1","1 < Cc < 3","Cc > 3"].map(x=><button className={ccMeaning===x?"selected":""} onClick={()=>setCcMeaning(x)} key={x}>{x}</button>)}</div></div><button className="primary check-coefficients" onClick={checkCoefficients}>VERIFICAR COEFICIENTES</button></div>}{answered>0&&<p className="success">✅ Cu = 3,75: solo uniforme. Cc = 1,28: está entre 1 e 3. Para areia bem graduada, a aula exige também Cu ≥ 4.</p>}</>}
